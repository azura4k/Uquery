...
<template :key="$route.fullPath">
  <div class="row" id="wrapper">
    <nav class="navbar navbar-dark align-items-start sidebar sidebar-dark accordion bg-gradient-primary p-2 hide toggled">
      <div class="container-fluid d-flex flex-column p-0"><router-link tag="a"
          class="navbar-brand d-flex justify-content-center align-items-center sidebar-brand m-0" to="/">
          ...
          <div class="container-fluid"><button class="btn btn-link d-md-none rounded-circle mr-3" id="sidebarToggleTop"
            ...
            <ul class="navbar-nav flex-nowrap ml-auto">
              ...
              <!-- <li class="nav-item dropdown no-arrow">
                <div v-if="LoggedIn" class="nav-item dropdown no-arrow"><a class="dropdown-toggle nav-link" aria-expanded="false"
                    data-toggle="dropdown" href="#"><span
                      class="d-none d-lg-inline mr-2 text-gray-600 small">{{this.Username}}</span><img
                      class="border rounded-circle img-profile" :src="GravatarPfP"></a>
                </div>
              </li> -->
            </ul>
          </div>
        </nav>
          <RouterView :key="$route.fullPath" @UserLoggedIn="ReloadNavBar()" />
    ...
    methods: {
      ...
      ReloadNavBar(){
         if (this.$cookies.isKey("Token")){
          this.LoggedIn = true
          this.GravatarPfP = this.$cookies.get("Gravatar")
